...
class RedisClient(object):
    ...
    def get(self, https):
        ...
        if https:
            ...
            # banned proxy is not selected;only useable proxy is selected
            proxies = list(filter(lambda x: json.loads(x).get("https") and not json.loads(x).get("ban") and json.loads(x).get("is_useable"), items))
            return choice(proxies) if proxies else None
        else:
            items = self.__conn.hvals(self.name)
            # banned proxy is not selected;only useable proxy is selected
            proxies = list(filter(lambda x: not json.loads(x).get("ban") and json.loads(x).get("is_useable"), items))
            print(json.dumps(proxies,ensure_ascii=False))
            return choice(proxies) if proxies else None
    ...
    def getValueByField(self, field):
        """
        根据field返回hash的value
        :param field: proxy str
        :return:
        """
        return self.__conn.hget(self.name, field) if field else None

    def updateValueByField(self,field,value):
        """
        根据field更新hash的value
        :param field: proxy str
        :param value: proxy object
        :return:
        """
        return self.__conn.hset(self.name, field, value)
